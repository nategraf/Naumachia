...
### Create a Challenge
1. Write a [`docker-compose.yml` template] and put it and any associated files in directory within
   the `challenges` directory. Check out the [example challenge] as a starting point.
2. Modify `config.yml` to include your challenge. If you don't yet have a config.yml, checkout
   [`config.example.yml`] for guidance.
3. Run `configure.py` to generate the `docker-compose.yml` file, OpenVPN config files, and PKI from
   a Jinja2 templates.

[`docker-compose.yml` template]: https://docs.docker.com/compose/compose-file/
[example challenge]: https://github.com/nategraf/Naumachia-challenges/tree/master/example
